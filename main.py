...
__author_desc__ = (
    "A humble graduate from King Fahd University of Petroleum and Minerals."
)
...
def create_bot() -> commands.Bot:
    ...
    intents: discord.Intents = discord.Intents(
        voice_states=True, messages=True, guilds=True
    )
    client: commands.Bot = commands.Bot(
        command_prefix="!", help_command=help_command, intents=intents
    )
    ...
    @client.command(
        name="init",
        aliases=["setup"],
        help="Initialize the bot to this server.",
    )
    @commands.has_guild_permissions(**command_permissions)
    async def initialize(ctx: commands.Context):
        setup_message: discord.Message = await ctx.send(
            "initializing bot in server..."
        )
